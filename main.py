...
def write_copy(rows, date, changes_exist):
    ...
    weibo = (
        f"#美股指数基金# {date_str} 标普500/纳指100 场外申购额度日报｜{headline}\n\n"
        f"{body_txt}\n\n"
    ...
    (OUT / f"weibo_{date.isoformat()}.txt").write_text(weibo, encoding="utf-8")
    (OUT / f"xhs_{date.isoformat()}.txt").write_text(xhs, encoding="utf-8")
